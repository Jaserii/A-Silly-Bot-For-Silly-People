...
public class PauseCmd implements IDiscordCommand {
    private static final Logger logger = LoggerFactory.getLogger(PauseCmd.class);

    private String name = "pause";
    private String desc = "(Un)Pause the currently playing track";

    private LavalinkService lavalinkService;

    public PauseCmd(LavalinkService lavalinkService) {
    ...
    public void execute(SlashCommandInteractionEvent commandEvent) {
        ...
        lavalinkService.getLavalinkClient().getOrCreateLink(guild.getIdLong())
                ...
                .subscribe((player) -> {
                    commandEvent.getHook().sendMessage("Player has been " + (player.getPaused() ? "paused" : "resumed") + "!").queue();
                });
    }
}
